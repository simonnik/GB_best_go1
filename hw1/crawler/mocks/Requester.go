// Code generated by mockery v2.9.4. DO NOT EDIT.

package mocks

import (
	context "context"

	mock "github.com/stretchr/testify/mock"
	crawler "github.com/t0pep0/GB_best_go1/hw1/crawler"
)

// Requester is an autogenerated mock type for the Requester type
type Requester struct {
	mock.Mock
}

// Get provides a mock function with given fields: ctx, url
func (_m *Requester) Get(ctx context.Context, url string) (crawler.Page, error) {
	ret := _m.Called(ctx, url)

	var r0 crawler.Page
	if rf, ok := ret.Get(0).(func(context.Context, string) crawler.Page); ok {
		r0 = rf(ctx, url)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(crawler.Page)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, string) error); ok {
		r1 = rf(ctx, url)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
